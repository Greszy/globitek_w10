<?php
  ...
  function validate_user($user, $errors=array()) {
    ...
    } elseif (!password_matches_confirmation($user['password'],$user['password_confirmation'])) {
  ...
  function insert_user($user) {
    ...
    $sql .= "'" . db_escape($db, hashed_password($user['password'])) . "',";
    $sql .= "'" . $created_at . "'";
    $sql .= ");";
    // For INSERT statements, $result is just true/false
    $result = db_query($db, $sql);
  ...
  function update_user($user) {
    ...
    $sql .= "hashed_password='" . hashed_password($user['password']) . "' ";
  ...
  function  find_failed_login($username){
    ...
    $sql .= "WHERE username = '" . db_escape($db, $username) . "';";
    $failed_result = db_query($db, $sql);
    return $failed_result;
  }

  // insert login-failed user 

  function insert_failed_login($failed_login = array()){
    global $db;
  ...
  function update_failed_login($failed_login){
    ...
    $sql .= "WHERE id='" . db_escape($db, $failed_login['id']) . "' ";
    $sql .= "LIMIT 1;";
    // For update_user statements, $result is just true/false
    $result = db_query($db, $sql);
    if($result) {
